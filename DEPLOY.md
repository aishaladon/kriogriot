...
| `NAS_HOST` / `NAS_USER` / `NAS_PASS` / `NAS_FOLDER` | Optional — see `NAS-FTPS.md`. `NAS_HOST` accepts a bare hostname or a full `ftps://host:port` URL. Unset means uploads are simply not mirrored to the NAS; nothing else breaks. |
